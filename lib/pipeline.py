...
class SubmitCancelledError(Exception):
    """Raised when the user cancels submission at the confirm prompt."""
...
class AutoPipeline:
    ...
    def _s3_bio(self):
        ...
        if self._stopped():
            return
        try:
            existing = scrape_profile_form(self.s)
            bio      = existing.get("bio", "").strip()
            location = existing.get("location", "").strip()
            if bio and location:
                self._log(f"\u2714 Bio already set: {bio[:60]}", "ok")
                self._log(f"\u2714 Location already set: {location}", "ok")
                return
            if not bio:
    ...
    def _s5_billing(self, fn, ln):
        ...
        try:
            ...
            eb = {}
        # ── Skip if billing already filled ───────────────────────────
        if eb.get("address1") and eb.get("city"):
            city   = eb.get("city", "")
            region = eb.get("region", "")
            addr1  = eb.get("address1", "")
            self._log(f"\u2714 Billing already set: {city} ({region})", "ok")
            self._log(f"  {addr1[:70]}", "info")
            return
        cfg = load_settings()
    ...
    def _s10_submit(self, chosen, id_path):
        ...
        if self._ask_confirm_submit:
            ...
            if not confirmed:
                ...
                raise SubmitCancelledError("Submit cancelled by user.")
